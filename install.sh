#!/bin/sh
# Fetch the raw grimoire-setup installer from the latest release and run it.
#
...
#
# GRIMOIRE_VERSION pins a release tag (e.g. GRIMOIRE_VERSION=v0.4.0 ... | sh);
# unset, the latest release installs. The variable goes before `sh`, not `curl`.
set -eu

REPO="chinese-room-solutions/grimoire"
ASSET_PREFIX="grimoire-setup"
if [ -n "${GRIMOIRE_VERSION:-}" ]; then
    BASE_URL="https://github.com/$REPO/releases/download/$GRIMOIRE_VERSION"
else
