...
func TestCLIReindex(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantBody: `{"force":false,"paths":null}`,
			wantExit: exitOK,
		...
		},
		{
			...
			wantBody: `{"force":true,"paths":null}`,
			wantExit: exitOK,
		...
		{
			name:     "positional paths narrow the pass",
			args:     []string{"reindex", "a.md", "sub/b.md"},
			respond:  map[string]any{"indexed": 2, "skipped": 0, "pruned": 0, "chunks": 6, "failed": 0},
			wantBody: `{"force":false,"paths":["a.md","sub/b.md"]}`,
			wantExit: exitOK,
			wantOut:  "indexed 2, skipped 0, pruned 0, chunks 6\n",
		},
		{
			name:     "a path with --force re-embeds that note",
			args:     []string{"reindex", "--force", "a.md"},
			respond:  map[string]any{"indexed": 1, "skipped": 0, "pruned": 0, "chunks": 3, "failed": 0},
			wantBody: `{"force":true,"paths":["a.md"]}`,
			wantExit: exitOK,
			wantOut:  "indexed 1, skipped 0, pruned 0, chunks 3\n",
		},
	}
