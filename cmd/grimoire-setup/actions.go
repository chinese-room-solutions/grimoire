...
// runInstall stages the app, creates a launcher, and records the install. Unlike
// MASS there is no config to write — Grimoire is configured at runtime in the UI.
// mode selects the end screen: themed Back/Exit for the wizard, or plain stdout
// lines for the scripted/elevated-child face.
...
// doInstall installs, drawing the step list as it goes: without it the form just
// sits there while files are copied and the next thing the operator sees is the
// result screen. Grimoire has no settings to save first — the vault is picked on
// first run — so the page opens straight into the install itself. The phase page is
// closed before this returns, so the result screen doesn't nest inside it.
...
func openPhase(tag string, mode endMode) (*term.Phase, func()) {
	if mode == endWizard {
		return tui.OpenPhase(grimoireArt, tag, term.TerminalWidth())
	}
...
// the summary already carries where Grimoire landed, how to launch it and where its
// vault comes from; these rows say only what happened. A best-effort step that
// failed (the launcher, PATH) marks itself ✖ and the install carries on — Install
// doesn't treat it as fatal either, since the binary is staged and runnable.
func reportStep(ph *term.Phase, installDir string, step install.Step, err error) {
	...
	case install.StepStage:
		...
		ph.Line(term.OKMark() + "Staged Grimoire into " + installDir)
...
// The confirm keeps the wizard's look over the step, and a declined prompt cleanly
// returns to the form instead of erroring. In the scripted face (not the wizard)
// it skips the prompt and relaunches directly. verb is the action word for the
// prompt ("Installing to" / "Removing from").
func elevate(verb, dir string, relaunchArgs []string, tag string, mode endMode) elevationResult {
	if mode == endWizard {
		if choice, err := tui.Confirm(grimoireArt, tag,
			[]string{
	...
	case install.ElevatedChildStarted, install.ElevatedWorkSucceeded:
		// Windows: a child does the work in its own UAC window. POSIX: sudo ran
		// the work synchronously just now. Either way this process reports the
		// result — it must not re-run the install itself.
...
func paintElevationNotice(tag string) {
	...
	b.WriteString(term.Banner(grimoireArt, tag, cols))
	b.WriteString("\n")
...
// installSummary is what an install leaves behind: where the app went and how to
// launch it. Grimoire opens to its empty state on first run, where the user picks a
// vault — so the summary points them there rather than at any config file.
//
...
func installSummary(c collected, launcherOK bool, cli install.CLIResult) term.Summary {
	...
	s := term.Summary{
		...
		Rows: []term.SummaryRow{
			...
			{Label: "vault", Value: "pick one in the app on first run (a folder of Markdown notes)"},
		},
	}
	if !launcherOK {
...
func runUninstall(installDir string, perUser bool, tag string, mode endMode) actionOutcome {
	...
	// a stray Enter on the form's Uninstall button can't remove Grimoire unprompted.
	...
	// elevated child re-runs). Grimoire's vaults and data live under the per-user
	// cache root, never inside the install dir, so they survive removal untouched.
	if mode == endWizard {
		if choice, err := tui.Confirm(grimoireArt, tag,
			[]string{
				"Remove Grimoire from " + installDir + "?",
				"Your vaults and data are left untouched.",
...
// all they keep of where Grimoire landed and how to launch it.
//
...
func finish(tag string, s term.Summary, mode endMode) actionOutcome {
	if mode == endWizard {
		if back, err := tui.BackOrExit(grimoireArt, tag, tui.SummaryRows(s, term.TerminalWidth())); err == nil {
			if back {
				return actionOutcome{back: true}
			}
...
func fail(tag, msg string, mode endMode) actionOutcome {
	...
	if mode == endWizard {
		if back, err := tui.ErrorScreen(grimoireArt, tag, msg); err == nil {
			if !back {
