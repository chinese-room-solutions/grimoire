...
-- Keyword index over the chunks. porter wraps unicode61 (which tokenizes
-- Latin and Cyrillic alike; remove_diacritics folds '´'/'' and accented
-- Latin), stemming every token so "slo" matches "slos" and "restart"
-- matches "restarting" — the exact-phrase gap measured on eval/ lost whole
-- queries (an "SLO" lookup never reached the note that says "SLOs"). Porter
-- applies English suffix rules to every token, so non-English words are
-- stemmed too — the vector leg stays morphology-agnostic and carries those
-- queries. path is a column so filename terms match ("my-note.md" tokenizes
-- to my, note, md).
